 ...
 */
SIM_Client::SIM_Client(int RX_pin, int TX_pin, int PWR_pin, HardwareSerial &serial_channel, Point &sensor):_serialAT(serial_channel),_sensor(sensor){
    _RX_pin = RX_pin;
    _TX_pin = TX_pin;
    _PWR_pin = PWR_pin;
...
/**
 * Envía la estructura de datos con los valores de sensores de temperatura, voltaje y corriente a la base de datos mediante https separándola en bytes.
 * @param data Datos a enviar en el formato de struct API_data.
 * @return true si se envió correctamente, false si algo falló
 */
bool SIM_Client::send_http(API_data data){
...
/**
 * Envía la estructura de datos con los valores de sensores de temperatura, voltaje y corriente a la base de datos de InfluxDB separándola en bytes.
 * @param data Datos a enviar en el formato de struct API_data.
 * @return true si se envió correctamente, false si algo falló
 */
bool SIM_Client::send(API_data data){

    uint16_t* ptr = (uint16_t*)&data; //Casting del puntero de la estructura a un puntero de 8 bits.
    ...
    _sensor.addField("Temperatura BP", *ptr++);
    _sensor.addField("Temperatura BS", *ptr++);
    _sensor.addField("V_BP", *ptr++);
    _sensor.addField("V_BS", *ptr++);
    _sensor.addField("V_Motor", *ptr++);
    _sensor.addField("C_BP", *ptr++);
    _sensor.addField("C_BS", *ptr++);
    _sensor.addField("C_Motor", *ptr++);
